...
if __name__ == "__main__":
            ...
            for vote in  event["active_votes"]:
                if vote["voter"] == vote_event["voter"] and vote["rshares"] < 0:
                    if start_rshares + float(vote["rshares"]) < 0:
                        print vote["time"],"FLAG",vote["voter"],"=>",vote_event["author"],vote["rshares"],"(",start_rshares , "->", start_rshares + float(vote["rshares"]) , ")" 
                    else:
                        print vote["time"],"DV",vote["voter"],"=>",vote_event["author"],vote["rshares"],"(",start_rshares , "->" , start_rshares + float(vote["rshares"]) , ")"
    ...
    def get_block(blk):
        ...
        def process_block(event, client):
            ...
            print "get_block",nextblock,len(client.queue),client.active_call_count
            get_block(nextblock)
